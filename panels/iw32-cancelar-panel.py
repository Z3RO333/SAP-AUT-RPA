...
from core.iw32.cancelar import profile_default_service, run_job


class Iw32CancelarPanel:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Robo SAP IW32 - Cancelar Ordens")
        ...
        self.motivo_var = tk.StringVar(value="LOJA FECHADA")
        self.num_servico_var = tk.StringVar(value=profile_default_service())
        self.status_var = tk.StringVar(value="Pronto. Informe ordens, motivo e execute.")
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(header, text="Robo SAP IW32 - Cancelar Ordens", style="HeaderTitle.TLabel").place(x=24, y=18)
        ttk.Label(
            header,
            text="Cancela ordens em lote no IW32 pela aba +CUK, com motivo e numero de servico.",
        ...
        ttk.Label(form, text="Motivo", style="Muted.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Entry(form, textvariable=self.motivo_var, width=40).grid(row=1, column=0, sticky="we", padx=(0, 12), pady=(4, 10))

        ttk.Label(form, text="Numero de servico", style="Muted.TLabel").grid(row=0, column=1, sticky="w")
        ttk.Entry(form, textvariable=self.num_servico_var, width=16).grid(row=1, column=1, sticky="w", pady=(4, 10))
    ...
    def _run(self, orders: list[str]) -> None:
        try:
            result = run_job(
                {
                    "orders": orders,
                    "reason": self.motivo_var.get().strip(),
                    "service_number": self.num_servico_var.get().strip(),
                },
                {
    ...
    def _clear(self) -> None:
        ...
        for item in self.tree.get_children():
            self.tree.delete(item)
        self.status_var.set("Pronto. Informe ordens, motivo e execute.")


def main() -> int:
    root = tk.Tk()
    Iw32CancelarPanel(root)
    root.mainloop()
    return 0
