...
from core.reports.ordens_aberto import apply_filters, load_report


class OrdensAbertoPanel:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Ordens em Aberto")
        ...
        self.status_var = tk.StringVar(value="Pronto. Selecione uma planilha e aplique os filtros desejados.")
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(header, text="Ordens em Aberto", style="HeaderTitle.TLabel").place(x=24, y=18)
        ttk.Label(header, text="Filtra uma planilha local de ordens abertas e exporta Excel/PDF.", style="HeaderText.TLabel").place(x=26, y=56)
        ...
        ttk.Label(form, text="Centro", style="Muted.TLabel").grid(row=2, column=0, sticky="w")
        ttk.Entry(form, textvariable=self.centro_var, width=18).grid(row=3, column=0, sticky="w", padx=(0, 10), pady=(4, 10))
        ttk.Label(form, text="Data inicial (YYYY-MM-DD)", style="Muted.TLabel").grid(row=2, column=1, sticky="w")
        ttk.Entry(form, textvariable=self.date_from_var, width=18).grid(row=3, column=1, sticky="w", padx=(0, 10), pady=(4, 10))
        ttk.Label(form, text="Data final (YYYY-MM-DD)", style="Muted.TLabel").grid(row=2, column=2, sticky="w")
        ttk.Entry(form, textvariable=self.date_to_var, width=18).grid(row=3, column=2, sticky="w", pady=(4, 10))
    ...
    def _run(self) -> None:
        try:
            ...
            output_dir = ensure_dir(run_output_dir("OrdensAbertas", business_key=Path(source).stem))
            ...
            if filtered.empty:
                ...
                return
            excel_path = export_dataframe_excel(filtered, output_dir / "ordens_abertas_filtradas.xlsx")
            pdf_path = export_dataframe_pdf(filtered, output_dir / "ordens_abertas_filtradas.pdf", "Ordens em Aberto")
