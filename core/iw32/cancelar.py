...
def cancelar_ordem(session, order: str, reason: str, service_number: str, profile: dict, context: RunContext) -> dict:
    open_order_iw32(session, order, profile, context=context)
    select_cuk_tab(session, profile, context=context)
    motivo_field = profile.get("fields", {}).get("motivoText", [])
    service_field = profile.get("fields", {}).get("serviceNumber", [])
    cancel_button = profile.get("buttons", {}).get("cancel", [])
    save_button = profile.get("buttons", {}).get("save", [])

    if motivo_field:
        set_text(session, motivo_field, reason, context=context)
    if service_field and service_number:
        set_text(session, service_field, service_number, context=context)
    press(session, cancel_button, context=context)
    close_popup_ok(session)
    if save_button:
        press(session, save_button, context=context)
    status_text, status_type = read_statusbar(session)
    if status_type == "E":
        raise RuntimeError(status_text or f"Falha ao cancelar a ordem {order}.")
...
def run_job(input_data, options, callbacks) -> RobotResult:
    ...
    run_id = timestamp_id()
    output_dir = ensure_dir(Path(options.get("output_dir") or run_output_dir("IW32-Cancelar", run_id=run_id)))
    ...
    result = RobotResult.new(robot="IW32-CANCELAR", run_id=run_id)
    orders = [str(item).strip() for item in input_data.get("orders", []) if str(item).strip()]
    reason = str(input_data.get("reason", "LOJA FECHADA")).strip() or "LOJA FECHADA"
    service_number = str(input_data.get("service_number", profile_default_service())).strip()
    payload_path = output_dir / "payload.json"
    payload_path.write_text(
        json.dumps({"orders": orders, "reason": reason, "service_number": service_number}, indent=2),
        encoding="utf-8",
    )
    ...
    try:
        ...
        for index, order in enumerate(orders, start=1):
            if context.is_cancelled():
                ...
                break
            context.progress("iw32-cancelar", index, len(orders))
            try:
                results.append(cancelar_ordem(session, order, reason, service_number, profile, context))
...
def profile_default_service() -> str:
    _profile_name, profile = load_iw32_profile()
    return str(profile.get("defaults", {}).get("serviceNumber", "3000465"))
